...
local function createLMDB(dataPath, lmdbPath)
  ...
  local dbUttid, readerUttid = startWriter(lmdbPath .. '/uttid', 'uttid') -- utterance id (e.g. mwjg0_sx404)
  ...
  local function getData(opts)
    ...
    -- get utterance id
    local uttid = paths.basename(opts.audioFilePath):gsub(extension, "")
    
    return { spect, transcript, uttid }
  end
  ...
  closeWriter(dbUttid, readerUttid)
end
