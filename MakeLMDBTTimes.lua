...
cmd:option('-timesExtension', 'endtimes', 'The extension of the time stamp files')
...
local timesExtension = '.' .. opt.timesExtension
...
local function createLMDB(dataPath, lmdbPath)
  ...
  local function getSize(opts)
    ...
    local timesFilePath = opts.file:gsub(opts.extension, opts.timesExtension)
    local opt = opts.opt
    ...
    return { audioFilePath, transcriptFilePath, length, timesFilePath }
  end

  for x = 1, opt.processes do
    local opts = { extension = extension, file = buffer[x], opt = opt, timesExtension = timesExtension }
  ...
  for x = 1, size do
    ...
    if x + opt.processes <= size then
      local opts = { extension = extension, file = buffer[x + opt.processes], opt = opt, timesExtension = timesExtension }
  ...
  local dbTimes, readerTimes = startWriter(lmdbPath .. '/times', 'times')
  ...
  local function getData(opts)
    ...
    local times
    for line in io.lines(opts.timesFilePath) do       
      times = line
    end
    -- TODO is tensor the best way to store the times?
    -- TODO: currently saving the frame number. Should we do some alignment with the windows or leave it for the model? 
    times = torch.LongTensor(times:split(' '))
    
    return { spect, transcript, times }
  end

  for x = 1, opt.processes do
    local vec = vecs[x]
    local opts = { audioFilePath = vec[1], transcriptFilePath = vec[2], opt = opt, timesFilePath = vec[4] }
  ...
  closeWriter(dbTimes, readerTimes)
end
...
function parent()
  ...
  if paths.dirp(dataPath .. '/dev') then
    createLMDB(dataPath .. '/dev', lmdbPath .. '/dev')
  end
  parallel.close()
end
